...
def evaluate_board(board):
    """
    Heuristic evaluation function that assigns values to pieces and returns a score for the board.
...
def minimax(board, depth, is_maximizing, alpha, beta):
    ...
    """
    if depth == 0 or is_checkmate(board, "white") or is_checkmate(board, "black") or is_stalemate(board, "white") or is_stalemate(board, "black"):
...
def ai_move(board):
    """
    Executes the AI's move on the board by choosing the optimal move using minimax.
    """
    best_move = None
    best_value = -math.inf
    for move in get_all_possible_moves(board, "white"):
        new_board = simulate_move(board, move)
        board_value = minimax(new_board, 3, False, -math.inf, math.inf)  # Depth 3 for intermediate difficulty
